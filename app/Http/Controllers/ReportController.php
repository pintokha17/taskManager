    ...
    public function index(Request $request)
    {
        ...
        if ($request->isMethod('POST'))
        {
            ...
            $tasks = DB::table('tasks')
                ...
                ->whereRaw("GREATEST(start, '{$dateFromFormatted}') AND (LEAST(pause, '{$dateToFormatted}') OR pause IS NULL) AND (start <= '{$dateToFormatted}' AND pause >= '{$dateFromFormatted}' OR pause IS NULL)")
